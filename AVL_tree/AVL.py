...
class AVLNode(Node):
    def __init__(
        self, value, parent=None, frequency=1, left_node=None, right_node=None, height=1
    ) -> None:
        super().__init__(
            value, frequency=frequency, left_node=left_node, right_node=right_node
        )
        self.height = (
            height  # definition: max(self.left_node.height, self.right_node.height) + 1
        )
        self.parent = parent
        self.balancing_factor = (
            0  # definition: self.right_node.height - self.left_node.height
        )
...
class AVLTree:
    ...
    def update_height(self, node) -> None:
        ...
        node.height = (
            max(
                0 if node.left_node == None else node.left_node.height,
                0 if node.right_node == None else node.right_node.height,
            )
            + 1
        )
    ...
    def _higher_subtree_child(node):
        ...
        if node.left_node != None and node.right_node != None:
            return (
                node.left_node
                if node.left_node.height >= node.right_node.height
